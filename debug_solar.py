...
def test_basic_solar_calculation():
    ...
    print(f"Tomorrow 6 AM: elevation={pos_6am['elevation']:.2f}°, azimuth={pos_6am['azimuth']:.2f}°")
    print(f"Tomorrow noon: elevation={pos_noon['elevation']:.2f}°, azimuth={pos_noon['azimuth']:.2f}°")
    
    # Calculate approximate sunrise/sunset for tomorrow
    print("\n=== Approximate Sunrise/Sunset ===")
    for hour in range(0, 24):
        test_time = (now + timedelta(days=1)).replace(hour=hour, minute=0, second=0, microsecond=0)
